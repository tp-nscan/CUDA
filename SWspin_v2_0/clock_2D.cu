...
//    3. 2D q-state clock model
//
//    model: q-state Potts model on the square lattice
//           with periodic boundary conditions
//    system size: nx*nx  (nx should be a multiple of 32;
//                 maximum size of nx depends on memory size)
//    number of states: q  (q should be less than or equal to 512)
...
__global__ void device_function_spin_flip  (int*, int*, unsigned int*,int);
__global__ void device_function_spin_flip_YK(int*, int*, unsigned int*,int );
__global__ void device_function_sum        (int*, double*, double*,  
                                            double*, double*, double*   );


#define nx 128  // linear system size
#define q  512  // number of clock states
#define PI M_PI
...
/*------------ Size of array ------------------------------------------*/
    ...
    unsigned int mem_rand              = sizeof(double)*(3*nla);
    unsigned int mem_1                 = sizeof(int)*(1);
    unsigned int mem_q                 = sizeof(double)*(q);
    unsigned int mem_measured_quantity = sizeof(double)*(grid);
    unsigned int mem_measured_magnet   = sizeof(double)*(grid*2);


/*------------ Assignment of memory for host --------------------------*/

    int* h_flag        = (int*) malloc(mem_1);
    double* h_out_energy = (double*) malloc(mem_measured_quantity);
    double* h_out_magnet = (double*) malloc(mem_measured_magnet  );
...
/*------------ Assignment of memory for device ------------------------*/
    ...
    cudaMalloc((void**) &d_rule       , mem_q );
    cudaMalloc((void**) &d_fmx        , mem_q );
    cudaMalloc((void**) &d_fmy        , mem_q );
    cudaMalloc((void**) &d_out_energy , mem_measured_quantity);
    cudaMalloc((void**) &d_out_magnet , mem_measured_magnet  );
    cudaMemcpy(d_rule,rule,mem_q,cudaMemcpyHostToDevice);
    cudaMemcpy(d_fmx ,fmx ,mem_q,cudaMemcpyHostToDevice);
    cudaMemcpy(d_fmy ,fmy ,mem_q,cudaMemcpyHostToDevice);
...
/*------------ Temperature and Monte Carlo steps ----------------------*/
    ...
    const double  tstart = 0.82, tunit = 0.02;
    const int    tnum = 8;

    const int mcs1 = 5000, mcs2 = 20000;

    int ispr; // mirror axis for Wolff's embedded cluster

/*------------ Variables for measured quantities ----------------------*/

    double H, M, fmxsum, fmysum, fm[5];
...
/*------------ Equilibration ------------------------------------------*/
      ...
      for(int mcs=1; mcs <= mcs1; mcs++){
        ...
        ispr = abs(rand())%q;
        curandGenerateUniformDouble(gen, d_random_data, 3*nla);

        // Bond connection
        if(algorithm==0){
          device_function_init_H<<<grid,threads>>>(t, d_spin, d_random_data, 
                 d_label, d_R,  d_rule, d_fmy, ispr);
        } else if(algorithm==1){
          device_function_init_K<<<grid,threads>>>(t, d_spin, d_random_data, 
                 d_label,  d_rule, d_fmy, ispr);
        } else if(algorithm==2){
          device_function_init_YK<<<grid,threads>>>(t, d_spin, d_bond, 
                 d_random_data, d_label,  d_rule, d_fmy, ispr);     
        }
...
/*------------ Measurement --------------------------------------------*/
        ...
        device_function_sum<<<grid,threads>>>(d_spin, d_fmx, d_fmy, d_rule, 
                        d_out_energy, d_out_magnet);
        ...
        for(int i=0; i<grid; i++){
          H  += h_out_energy[i];
          fmxsum  += h_out_magnet[  2*i];
          fmysum  += h_out_magnet[1+2*i];
        }
        M = fmxsum*fmxsum+fmysum*fmysum;
        fm[1] += M;
        fm[2] += M*M;
        fm[3] += H;
        fm[4] += H*H;
...
void ruleset()
/*
       table of energy for q-state clock model
*/
{
    for(int iq=0; iq<=q-1; iq++){
      rule[iq] = -cos(2*PI*iq/(double)q);
    }
}
...
void mset()
/*
       table of magnetization for q-state clock model
*/
{
    for(int iq=0; iq<=q-1; iq++){
      fmx[iq] = cos(PI*(2*iq+1)/(double)q);
      fmy[iq] = sin(PI*(2*iq+1)/(double)q);
    }
}
...
__global__ void device_function_init_H(double d_t, int* d_spin, 
                ...
                double* d_rule, double* d_fmy, int d_ispr)
/*
     Bond connection
        (Hawick algorithm)
*/
{
    unsigned int index = blockIdx.x * blockDim.x + threadIdx.x;
    unsigned int la;
    int ispr, spin0, spin00, spin1,i, bond;
    double t;
    double boltz;
    double e1, e2, fm1, fm2;
    __shared__ float rule[q], fmy[q];

    if(threadIdx.x < q){
      rule[threadIdx.x] = d_rule[threadIdx.x];
      fmy [threadIdx.x] = d_fmy [threadIdx.x];
    }
    __syncthreads();
    ...
    t    = d_t;
    spin0  = d_spin[index];
    spin00 = (q-1-spin0+2*ispr) % q;

/*------------ Bond connection with right site ------------------------*/

    for(i=0; i<2; i++){
     if(i==0)la = (index-1+nx)%nx+((int)(index/nx))*nx;
     if(i==1)la = (index-nx+nla)%nla;

     spin1 = d_spin[la];
     fm1 = fmy[(spin0-ispr+q)%q] ;
     fm2 = fmy[(spin1-ispr+q)%q] ;

     if( ( fm1*fm2 ) > 0 ){
       e1 = rule[(spin0 -spin1+q) % q];
       e2 = rule[(spin00-spin1+q) % q];
       boltz = exp(-(e2-e1)/t);
...
*/
{
    unsigned int index = blockIdx.x * blockDim.x + threadIdx.x;
    unsigned int la;
    int ispr, spin0, spin00, spin1,i, bond, index_min;
    double t;
    double boltz;
    double e1, e2, fm1, fm2;
    __shared__ float rule[q], fmy[q];
...
/*------------ Comparison with "label"s of left and top sites -----------*/
    ...
    for(i=0; i<2; i++){
     if((label>>i)&1==1){
      if(i==0) la = (index-1+nx)%nx+((int)(index/nx))*nx;
      if(i==1) la = (index-nx+nla)%nla;
      label_2 = min((d_label[la]>>2),label_2);
     }
    }
...
/*------------ Comparison between old and new "label"s ----------------*/
    ...
    if(label_2 < label_1){
      ...
      d_label[label_1] = (t_label&0x03) | ( label_3<<2 );
      d_flag[0] = 1;
    }
}
...
*/
{
    ...
    label = d_label[index];
    d_label[index] = (label&0x03) |(d_R[label>>2] <<2);
}
...
/*------------ Comparison with "label" of left site -------------------*/
...
/*------------ Comparison with "label" of top boudanry site -------------------*/
...
*/
{
    ...
    if( 0.5 < d_random_data[index+2*nla] ) {
      spin_new = 1;
    } else {
      spin_new = 0;
    }
...
__global__ void device_function_spin_flip(int* d_spin, int* d_spin_new, 
                unsigned int* d_label, int d_ispr)
/*
...
*/
{
    ...
    if( d_spin_new[( d_label[index]>>2 )] == 0 ) {
      d_spin[index] = ((q-1-d_spin[index]+2*d_ispr) % q);
    }
}
...
//****************************************************************************
__global__ void device_function_sum(int* d_spin, double* d_fmx, double* d_fmy,
                double* d_rule, double* out_energy, double* out_magnet)
...
*/
{
    unsigned int index = blockIdx.x * blockDim.x + threadIdx.x;
    int la[2], spin0;
    volatile __shared__ double dH[512], dM[512];
    __shared__ double rule[q], fmx[q], fmy[q];
    unsigned int dx;


    if(threadIdx.x < q){
      rule[threadIdx.x] = d_rule[threadIdx.x];
      fmx[threadIdx.x]  = d_fmx[threadIdx.x];
      fmy[threadIdx.x]  = d_fmy[threadIdx.x];
    }
    __syncthreads();
...
/*------------ Calculation of energy ----------------------------------*/
    ...
    spin0 = d_spin[index];
    dH[threadIdx.x] = 0;
    dH[threadIdx.x] += rule[(spin0-d_spin[la[0]]+q) % q];
    dH[threadIdx.x] += rule[(spin0-d_spin[la[1]]+q) % q];
    __syncthreads();
...
/*------------ Calculation of magnetization ---------------------------*/
    ...
    for(int i=0; i<=1; i++){
      ...
      dM[threadIdx.x] = (1-i)*fmx[spin0]+i*fmy[spin0];
      __syncthreads();
      ...
      if(threadIdx.x==0){ 
        out_magnet[i+2*blockIdx.x]=dM[0];  
      }
      __syncthreads();
    }
}
