...
//    6. 3D q-state clock model
//
//    model: q-state clock model on the cubic lattice
...
/*------------ Temperature and Monte Carlo steps ----------------------*/
    ...
    const double  tstart = 2.14, tunit = 0.02;
    const int    tnum = 8;
...
*/
{
    ...
    if( d_spin_new[( d_label[index]>>3 )] == 0 ) {
...
/*------------ Calculation of energy ----------------------------------*/
    ...
    dH[threadIdx.x] += rule[(spin0-d_spin[la[2]]+q) % q];
    __syncthreads();
