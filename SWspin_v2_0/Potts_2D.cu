...
//              (2-state Potts model is equivalent to Ising model)
...
__global__ void device_function_spin_select(int*, unsigned int*         );
...
/*------------ Size of array ------------------------------------------*/
    ...
    unsigned int mem_rand              = sizeof(double)*(2*nla);
    unsigned int mem_rand_uint         = sizeof(unsigned int)*(nla);
    unsigned int mem_1                 = sizeof(int)*(1);
    unsigned int mem_measured_quantity = sizeof(int)*(grid);
    unsigned int mem_measured_magnet   = sizeof(int)*(grid*q);
...
/*------------ Assignment of memory for device ------------------------*/
    ...
    unsigned int* d_random_uint_data;
    int* d_flag;
    int* d_out_energy;
    int* d_out_magnet;
    ...
    cudaMalloc((void**) &d_random_uint_data, mem_rand_uint);
...
/*------------ Temperature and Monte Carlo steps ----------------------*/
    ...
    const double tstart = 0.96, tunit = 0.01;
    const int    tnum = 8;
...
/*------------ Variables for measured quantities ----------------------*/
    ...
    double H, fn[q], f2order, fm[5];
      // fn[iq]: number of spins which are the state of iq (0:q-1)
...
/*------------ Equilibration ------------------------------------------*/
      ...
      for(int mcs=1; mcs <= mcs1; mcs++){
        ...
        curandGenerateUniformDouble(gen, d_random_data, 2*nla);
        curandGenerate(gen, d_random_uint_data, nla);
...
*/
{
    unsigned int index = blockIdx.x * blockDim.x + threadIdx.x;
    int spin_new;
    spin_new = d_random_uint_data[index]%q;
...
{
    unsigned int index = blockIdx.x * blockDim.x + threadIdx.x;
    d_spin[index] = d_spin_new[ ( d_label[index]>>2 ) ];
}
...
/*------------ Calculation of energy ----------------------------------*/
    ...
    if(S != d_spin[la[0]] )  {dH[threadIdx.x]++;}
    if(S != d_spin[la[1]] )  {dH[threadIdx.x]++;}
    __syncthreads();
...
/*------------ Calculation of magnetization ---------------------------*/
    ...
    for(int iq=0; iq<=q-1; iq++){
      ...
      if(threadIdx.x==0){
        out_magnet[iq+q*blockIdx.x] = dM[0];
      }
      __syncthreads();
    }
}
