...
__global__ void ticky_kernel(float *iptr, int tic) {
	// map from threadIdx/BlockIdx to pixel position
	int x = threadIdx.x + blockIdx.x * blockDim.x;
	int y = threadIdx.y + blockIdx.y * blockDim.y;
	int offset = x + y * blockDim.x * gridDim.x;
...
void anim_gpu(DataBlock *d, int ticks) {
	...
	for (int i = 0; i<100; i++) {
		...
		//ticky_kernel <<<blocks, threads >>>(in, ticks);
...
float *SetupHeaters(int imageSize)
{
...
{
	float *temp = (float*)malloc(arraySize);
	for (int i = 0; i<DIM*DIM; i++) {
...
int main(void) {
	...
	HANDLE_ERROR(cudaBindTexture(NULL, texIn, data.dev_inSrc,
		imageSize));

	HANDLE_ERROR(cudaBindTexture(NULL, texOut, data.dev_outSrc,
		imageSize));

	// intialize the constant data
	float *temp = RndFloat0to1(imageSize);
